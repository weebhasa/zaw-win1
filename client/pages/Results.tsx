...
export default function ResultsPage() {
  ...
  const state = location.state as
    | { details: any[]; score: number; total: number; sessionIndex?: number; totalSessions?: number }
    | undefined;
  ...
  if (!state) {
    return (
      <div className="container max-w-3xl py-16 text-center">
        ...
        <p className="mt-2 text-muted-foreground">Please take the test first.</p>
        <div className="mt-6 flex justify-center gap-3">
          <Button onClick={() => navigate("/test?session=0")}>Start Test</Button>
            ...
            <div key={d.id} className="rounded-xl border p-4">
              ...
              <div className="mt-2 text-sm text-muted-foreground">
                <div className="grid gap-1">
                  ...
                  <div>Correct answer: <span className="font-medium">{formatAnswer(d.type, d.correctAnswer, d.options)}</span></div>
                  <div>Your answer: <span className="font-medium">{formatAnswer(d.type, d.userAnswer, d.options)}</span></div>
                </div>
                {d.explanation && (
                  <div className="mt-2 text-foreground">Explanation: {d.explanation}</div>
                )}
              </div>
            </div>
          );
...
function formatAnswer(type: string, value: any, options?: string[]) {
  ...
  if (type === "multiple") {
    if (typeof value === "number" && options) return options[value] ?? String(value);
    return String(value);
  }
